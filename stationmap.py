...
def stationmap_coords(
    ...
    m = Basemap(
        ...
        lat_0=event_lat,
        lon_0=event_lon,
        llcrnrlat=min(alllats) - 0.1 * difflat,
        urcrnrlat=max(alllats) + 0.1 * difflat,
        llcrnrlon=min(alllons) - 0.1 * difflon,
        urcrnrlon=max(alllons) + 0.1 * difflon,
        resolution='i',
    )
    ...
    m.drawparallels(
        np.arange(np.floor(min(alllats)), np.ceil(max(alllats)), 1), labels=[1, 1, 0, 0]
    )
    m.drawmeridians(
        np.arange(np.floor(min(alllons)), np.ceil(max(alllons)), 1), labels=[0, 0, 0, 1]
    )
    ...
    if stations is not None and channels is not None:
        ...
        for x, y in zip(xs, ys):
            ...
            if not dist or min(dist) > dmin:
                ...
                xyplotted.append((x, y))
    plt.legend(
        [SP, BB, INF, UK], ['Short period', 'Broadband', 'Infrasound', 'Unknown']
    )
    plt.show()
    return fig
