                    ...
                    if os.path.isdir(fullpath) or flag == 1:
                        ...
                        for filen in filenames:
                            ...
                            if '_waveforms_' in basename:
                                ...
                                temp = '%s_%s%s_%s' % (e1, evname, evdat, name)
        ...
        if cursor_output.fetchone() is not None:
            ...
            with open(csvfilename, perm) as csvfile:
                ...
                if perm == 'wb':
                    ...
                    writer.writerow(['iid', 'note', 'source', 'ref_id', 'pho_id', 'file location'])
                    ...
                    for temp in data:
                        ...
                        if pho_id is not None:
                            ...
                            filelist = filelist + ';' + newfilenrel
                            short_ref1 = '%s; %s' % (short_ref1, photographer)

                        if len(filelist) > 0:
                            if filelist[0] == ';':
                                filelist = filelist[1:]
                        writer.writerow([iid, note.encode('utf-8'), short_ref1, ref_id, pho_id, filelist])
                    ...
                    for a1 in aids:
                        ...
                        with open(acsvfilename, perm) as acsvfile:
                            ...
                            if perm == 'wb':
                                ...
                                awriter.writerow(['iid', 'note', 'source', 'ref_id', 'pho_id', 'file location'])
                            awriter.writerow([iid, note.encode('utf-8'), short_ref1, ref_id, pho_id, filelist])
                pho_id = None
                ref_id = None
                temp = None
...
def clean_references(newdbname, newifname, relpath):
    """
    Copy all references to one big file
    """
    # Connect to database
    connection = None
    connection = lite.connect(newdbname)
    ...
    with open(os.path.join(os.path.dirname(newdbname), 'references.csv'), 'wb') as csvfile:
        ...
        for ref_id in rids:
            ...
            if file_extension1 is not None:
                ...
                writer2.writerow([short_ref1, ref_id, long_ref1.encode('utf-8'), '', url1])
...
def make_eventsummary(eids, newdbname, newifname, shortfilen):
    ...
    with open(os.path.join(os.path.dirname(newdbname), 'Events.csv'), 'wb') as csvfile:
        ...
        writer.writerow(['UTC', 'UTC', 'int', 'str', 'str', 'str', 'str', 'decimal degrees', 'decimal degrees', 'km', 'decimal degrees', 'decimal degrees', 'decimal degrees', 'decimal degrees', 'm2', 'm2', 'm2', 'm2', 'm3', 'm3', 'm3', 'kg', 'kg', 'kg', 'm', 'm', 'm', 'm', 'm', 'm', 'int', 'int', 'km', 'boolean', 'int', 'km', 'boolean', 'integer', 'km', 'str', 'str'])
        ...
        for dat in data:
            ...
            evname = evinf['Name'].strip().replace(' ', '').replace(',', '')
            evdat = evinf['StartTime'].strftime('%d%b%Y')
            fulldir = os.path.join(shortfilen, '%s_%s%s' % (Eid, evname, evdat))
...
def main(newdbname=None, newifname=None, database='/Users/kallstadt/LSseis/landslideDatabase/lsseis.db',
    ...
    clean_references(newdbname, newifname, relpath)
